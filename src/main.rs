use std::{collections::btree_set::Intersection, thread::spawn};

use macroquad::{miniquad::gl::GL_INTERLEAVED_ATTRIBS, prelude::*};
...
impl Player {
    ...
    pub fn update(&mut self, dt: f32) {
        let x_mov = match (is_key_down(KeyCode::Left), is_key_down(KeyCode::Right)) {
            (true, false) => -1f32,
            (false, true) => 1f32,
            _ => 0f32,
        };
        self.rect.x += x_mov * dt * PLAYER_SPEED;
        if self.rect.x < 0f32 {
            self.rect.x = 0f32;
        }
...
async fn main() {
    let font = load_ttf_font("res/D2Coding-Ver1.3.2-20180524.ttf")
        .await
        .unwrap();
                        ...
                        if resolve_collision(&mut ball.rect, &mut ball.vel, &block.rect) {
                            ...
                            if block.lives <= 0 {
                                ...
                                if block.block_type == BlockType::SpawnBallOnDeath {
                                    spawn_later.push(Ball::new(player.rect.point()+vec2(player.rect.w*0.5f32 - BALL_SIZE*0.5f32,-50f32)));
                                }
                            }
                        }
                    }
